...
// Actions
import {VS_Select_Club,VS_Select_Club_Array} from '../../../actions/Career_By';

// vars
var Teams=[],TeamArray=[], SelectaTeam, ThisTeam=[];

export default class ByClub_Select extends React.Component {
	CreateTeamArray(Team){
		
			ThisTeam=[];
			this.props.BATTING.BattingObject.map((game,i)=>{
				if(game.Game_Playing_For == Team)
					{	
						ThisTeam.push({game})
						}
			...
			// Dispatch to Reducer
			this.props.dispatch(VS_Select_Club_Array(ThisTeam));
		}
	 ...
	 change(event){
		...
		// Dispatch to Reducer
	 	this.props.dispatch(VS_Select_Club(event.target.value));
	 }
