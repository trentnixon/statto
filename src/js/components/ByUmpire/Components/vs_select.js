...
// Actions
import {VS_Select_Umpire,VS_Select_Umpire_Array} from '../../../actions/Career_By';

// vars
var Teams=[],TeamArray=[], SelectaTeam, ThisTeam=[];

export default class ByUmpire_Select extends React.Component {
	CreateTeamArray(Team){
		
			ThisTeam=[];
			this.props.BATTING.BattingObject.map((game,i)=>{
				if(game.Game_Umpire == Team)
					{	
						ThisTeam.push({game})
						}
			...
			// Dispatch to Reducer
			this.props.dispatch(VS_Select_Umpire_Array(ThisTeam));
		}
	 ...
	 change(event){
		...
		// Dispatch to Reducer
	 	this.props.dispatch(VS_Select_Umpire(event.target.value));
	 }
  ...
  render() {
    return (
			<select class="form-control pull-right " onChange={this.change.bind(this)} >
				<option value="false" >Select a Umpire</option>
					{
