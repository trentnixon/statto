...
export default class BasicStats extends React.Component {
	...
	CreatePieCountArray(Player)
		{
			pie_Over_count=[];
			Player.map((game,i)=>{			
					pie_Over_count.push({year:game.Date,value:game.Bowling_BallsBowled})
				})
	}
	...
	Create_Line_Average_PerGame(Player)
		{
			...
			Player.map((game,i)=>{			
				if(game.Bowling_BallsBowled > 0)
					{
						Line_Average_PerGame.push({Date:game.Date,BallsBowled:game.Bowling_CareerBallsBolwed,Runs:game.Bowling_TotalRunsConceded, Wickets:game.Bowling_TotalWicketsTaken});
						}
			})
	}
	...
	createTablePlayedCountYears(Player)
		{
			...
			Player.map((game,i)=>{	
					if(game.Bowling_BallsBowled > 0 ){			
						Table_Over_Count.push({name:game.Against, value:game.Bowling_BallsBowled})
					}
				})
	}
	...
	componentWillMount(){ 
		...
		// Set Stage INT
		Count = this.props.BATTING.Facts["0"].Bowling_CareerBallsBolwed;
		...
		*/
		...
		Tab_Graph=[
			{chart:<Create_Pie_Count_Items 
						Data={pie_Over_count} 
						Title="Balls Bowled by  Years" />},
							
			{chart:<Create_Lines_Triple 
							Data={Line_Average_PerGame} 
							Title="Balls vs Runs vs Wickets" 
							Line1="BallsBowled"
							Line2="Runs"
							Line3="Wickets" 
							XaxisLabel="Date"
