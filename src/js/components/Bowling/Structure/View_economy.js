...
export default class BasicStats extends React.Component {	
	...
	CreatePieCountArray(Player)
		{
			...
			Player.map((game,i)=>{
					...
					if(game.Bowling_OversBowled_parsed > 0)
						...
						AddEconomy = AddEconomy + game.Bowling_Weekly_Economy_Rate
				...
				if(game.Bowling_OversBowled_parsed > 0)
					{
						Line_Average_PerGame.push({
								Date:game.Date,
								GameEconomy:game.Bowling_Weekly_Economy_Rate,
								CareerEconomy:game.Bowling_Career_Economy_Rate
							});
						}
			})
	}
	...
	createTablePlayedCountYears(Player)
		{
			...
			Player.map((game,i)=>{	
					if(game.Bowling_BallsBowled > 0 ){			
						Table_Over_Count.push({name:game.Against, value:game.Bowling_Weekly_Economy_Rate})
					}
				})
	...
	componentWillMount(){ 
		...
		// Set Stage INT
		Count = this.props.BATTING.Facts["0"].Bowling_Career_Economy_Rate;
		...
		Tab_List = [
			{chart:<Stage_Table_Count_Items 
						Title="Economy Rates by Opposition" 
						TableHeader={TH} 
