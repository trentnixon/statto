...
export default class BasicStats extends React.Component {
	...
	createPieCountYears(Player)
		{
			...
			Player.map((game,i)=>{	
					...
					if(game.Runs_parsed == 0 && game.BallsFaced != '' && game.Runs.indexOf("*") == -1){	
						...
						pie_yearcount.push({year:game.Date,value:game.BallsFaced})
					}
				})
	...
	CreateBolwingAverage(Player){
			...
			Player.map((game,i)=>{			
				if(game.Runs_parsed == 0 && game.BallsFaced != '' && game.Runs.indexOf("*") == -1){
					Line_average.push({Against:game.Against,Average: game.Bowling_Career_Average,CareerAverage:CareerAverage});
				}
			})
		}
